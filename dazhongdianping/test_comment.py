...
if __name__ == '__main__':
    data = {"moduleInfoList":[{"moduleName":"reviewlist","query":{"shopId":"537537","page":1}}],"pageEnName":"shopreviewlist"}
    # http: // m.dianping.com / isoapi / module?id = 537537 & page = 1
    response = requests.post(url,headers=headers,data=json.dumps(data))
    # ob_json = json.loads(response.text)
    # list_comments = ob_json.get('data').get('moduleInfoList')
    print(response.text)
