...
def getShopInfo2(driver,shop_id,shop_name):
    ...
    time.sleep(1)
    try:
        shop_address = driver.find_element_by_css_selector('#main > div > div.shop-brief > div').text
    except Exception:
        shop_address = ''
    print 'shop_address:{}'.format(shop_address)
    driver.find_element_by_css_selector('#main > div > div.shop-brief > a').click()
    # try:
    ...
    try:
        shop_time = driver.find_element_by_css_selector('#main > section.info-block.introduce > div.hotel-info > div:nth-child(2)').text.replace('\n','')
    except Exception:
        shop_time = ''
    print 'shop_time:{}'.format(shop_time)
    try:
        shop_info = driver.find_element_by_css_selector('#main > section.info-block.introduce > div.hotel-info').text.replace('\n','')
    except Exception:
        shop_info = ''
    print 'shop_info:{}'.format(shop_info)
    driver.back()
...
def getShopInfo(driver):
    ...
    for each in driver.find_elements_by_css_selector('#app > div > div.J_searchList > ul > li'):
        item_count += 1
        print '...正在查看第{}个酒店...'.format(item_count)
