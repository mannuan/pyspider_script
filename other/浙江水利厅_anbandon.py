...
class Handler(BaseHandler):
    ...
    def new_list_page(self, response):
        ...
        for each_line_tr in response.doc('table#ctl00_cphBody_ctl00_dt_itemlist>tr').items():
            ...
            for each_new_table in each_line_tr('table').items():
                ...
                self.crawl(href, callback=self.detail_page, save={"title": title, "time": time, "page_type": page_type},exetime=time.time() + random.randint(60 * 60, 12 * 60 * 60))  # 1h~12h
    ...
    def on_result(self, result):
        ...
        if len(rows) == 0:
            cur.execute(
                "insert into website(title,url,push_time,spider_time,come_from,context,page_type,type_id,source) values(%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                (result["title"], result["url"], result["push_time"], result["spider_time"], result["come_from"],
                 result["context"], result["page_type"], '17', '浙江水利厅'))
        conn.commit()
        cur.close()
        conn.close()