...
def getCommentInfo2(driver,shop_id,shop_name,comment_list_url,page):
    ...
    for each in driver.find_elements_by_css_selector('#review-list > div.review-list-container > div.review-list-main > div.reviews-wrapper > div.reviews-items > ul > li'):
        ...
        comment_img = list()
        try:
            review_pic = each.find_elements_by_css_selector('div > div.review-pictures')
...
def getShopInfo(driver):
    ...
    for each in driver.find_elements_by_css_selector('#app > div > div.J_searchList > ul > li'):
        ...
        try:
            shop_price = float(re.sub(r'[^\d.]*',r'',item.find_element_by_css_selector('div.item-content > div.item-comment > span:nth-child(3)').text))
        except Exception:
            shop_price = 0
        ...
        try:
            shop_comment = int(re.sub(r'[^\d]*',r'',item.find_element_by_css_selector('div.item-content > div.item-comment > span:nth-child(2)').text))
        except Exception:
            shop_comment = 0
