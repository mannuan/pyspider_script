...
class Handler(BaseHandler):
    ...
    def on_result(self, result):
        ...
        if len(rows) == 0:
            cur.execute(
                "insert into wanfangnew(title,abstract,author,unit,magezine,file_url,time,keyword,url,spider_time,context) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
            ...
            cur.execute(
                "update wanfangnew set title=%s,abstract=%s,author=%s,unit=%s,magezine=%s,file_url=%s,time=%s,keyword=%s,spider_time=%s,context=%s where url=%s",
