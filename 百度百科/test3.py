...
class Handler(BaseHandler):
    ...
    def index_page(self, response):
        ...
        if url.find('https://baike.baidu.com/error.html') == 0:
            ...
            ##############
            for each in response.doc('body > div.body-wrapper > div.content-wrapper > div > div.main-content > div.lemma-summary > div').items():
                context+="<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{}</p>".format(each.text())
            ##############
            info_html = response.doc('body > div.body-wrapper > div.content-wrapper > div > div.main-content > div.basic-info.cmn-clearfix').html()
            for i in re.findall('<[^ \"</>]+([ ]+[^<>]*)?>',info_html):
                info_html = info_html.replace(i,'')
            info_html = re.sub('[\f\n\r]+','',info_html)
            context += info_html
            ##############
            catalog_html = response.doc('body > div.body-wrapper > div.content-wrapper > div > div.main-content > div.lemmaWgt-lemmaCatalog').html()
            for i in re.findall('<[^ \"</>]+([ ]+[^<>]*)?>',catalog_html):
                catalog_html = catalog_html.replace(i,'')
            catalog_html = re.sub('[\f\n\r]+','',catalog_html)
            context += catalog_html
            ##############
            context += '</div>'
