...
class Handler(BaseHandler):
    ...
    def on_start(self):
        ...
        self.crawl("http://baike.baidu.com/item/钱塘江", save={'river_name': '钱塘江'}, callback=self.index_page)
    ...
    def index_page(self, response):
        ...
        if url.find('https://baike.baidu.com/error.html') == 0:
            ...
            context = self.filter_page('body > div.body-wrapper > div.content-wrapper > div > div.main-content')
            p = pq(context)
            p('div > div.top-tool').remove()
            p('div > dl > dd > span.lemma-pinyin').remove()
            context = '<div class=\"main-content\">{}</div>'.format(p.html(method='html'))
            context = re.sub(
                r'[ ]+(?!class|style)[^ \"</>=]+[ ]*=[ ]*?\"[^\"<>]*?\"', '', context)
            context = context.replace('>编辑<','><')
            context = context.replace('>锁定<', '><')
            context += "<link rel=\"stylesheet\" type=\"text/css\" href=\"baidubaike.css\">"
